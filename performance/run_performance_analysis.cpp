...
void printComparison(const std::string& name, double current, double previous, bool higher_is_better) {
    ...
    if (improved) std::cout << GREEN;
    else if (worsened) std::cout << RED;
    else std::cout << WHITE;

    std::cout << "(" << (percent_change > 0 ? "+" : "") << std::fixed << std::setprecision(2) << percent_change << "% vs " << previous << ")" << WHITE << "\n";
}

int main(int argc, char* argv[]) {
    // TODO: Needs refactoring in the future, the plan is for the backtester to pull in all data from a specified folder
    std::string dataset = (argc > 1) ? argv[1] : "./data/NQ_sample.csv";
    std::string datetime = getCurrentDateTime();

    // MAIN PERFORMANCE COMMAND
    std::string cmd = "perf stat -x, -e instructions,cycles,L1-dcache-loads,L1-dcache-load-misses,branches,branch-misses ./build/release/src/backtest " + dataset + " 2>&1";
    ...
    double old_throughput = 0.0, old_l1_miss = 0.0, old_branch_miss = 0.0, old_ipc = 0.0, old_rss = 0.0;
    ...
    if (file_in.is_open()) {
        ...
        if (!last_line.empty() && last_line.find("Throughput") == std::string::npos && last_line.find("---") == std::string::npos) {
    ...
    if (ans.empty() || ans == "Y" || ans == "y") {
        ...
        if (file_out.is_open()) {
            file_out << "|" << datetime << "|" << dataset << "|" 
                     << std::fixed << std::setprecision(4) << throughput << "|"
                     << std::setprecision(2) << l1_miss_pct << "|"
                     << branch_miss_pct << "|"
                     << ipc << "|"
                     << max_rss << "|" << notes << "|\n";
